(function($){
	...
	var list = null; //当前的container
	var lastPos = null;//上一个时刻的位置
	...
	$.fn.dragsort = function(options){
		...
		opts = $.extend({},$.fn.dragsort.defaults, options);//全局参数

		this.each(function(i,cont){//cont为容器
			var newList = new ClassDragSort(i,cont);
			newList.init();
			lists.push(newList);
		});
	...
	var ClassDragSort = function(i,cont){
		...
		this.pos = null; //保存每个元素现在的位置
		this.offset = null; //鼠标在拖动元素上的offset
		this.offfsetLimit = null; //可拖动区域边界
		this.container = cont;
		this.currentId = i;
	}
	...
	ClassDragSort.fn.extend(
	{
		init: function(){
			...
			//给容器绑定事件
			$(this.container).attr('data-listidx',this.currentId).mousedown(this.grabItem).bind('dragsort-uninit', this.uninit);
